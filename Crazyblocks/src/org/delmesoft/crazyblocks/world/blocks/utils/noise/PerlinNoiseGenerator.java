...
public class PerlinNoiseGenerator
{
    ...
    public double improvedNoise(double x, double y, double z)
    {
        ...
        int uc_x = fastFloor(x) & 255;
        int uc_y = fastFloor(y) & 255;
        int uc_z = fastFloor(z) & 255;

        // Relative location of the point in the unit cube
        double xo = x - fastFloor(x);
        double yo = y - fastFloor(y);
        double zo = z - fastFloor(z);
    ...
    private void initPerlin1()
    {
        ...
        for(i = 0; i < B; i++)
        {
            ...
            g1[i] = (float)(((random.nextDouble() * Integer.MAX_VALUE) % (B + B)) - B) / B;

            for(j = 0; j < 2; j++)
                g2[i][j] = (float)(((random.nextDouble() * Integer.MAX_VALUE) % (B + B)) - B) / B;
            normalize2(g2[i]);

            for(j = 0; j < 3; j++)
                g3[i][j] = (float)(((random.nextDouble() * Integer.MAX_VALUE) % (B + B)) - B) / B;
            normalize3(g3[i]);
        }
        ...
        {
            k = p[i];
            j = (int)((random.nextDouble() * Integer.MAX_VALUE) % B);
            p[i] = p[j];
            p[j] = k;
        }
